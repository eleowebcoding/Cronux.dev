...
def limpiar_archivos():
    ...
    for path in ["dist", "build", "__pycache__", "temp_gui", "temp_cli"]:
        if Path(path).exists():
            shutil.rmtree(path)
...
def compilar_instalador_final(gui_path):
    ...
    try:
        ...
        # Agregar assets si existe la carpeta
        ...
        # Configuraciones específicas por sistema
        if sistema == 'windows':
            cmd.append("--uac-admin")  # Solicitar permisos de administrador
            # Solo agregar version-file si existe
